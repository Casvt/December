...
# To-Do
1. Actions
	1. Add/improve filters in media_transcode action
	2. Handle interruption of the script in media_transcode action [reference](https://video.stackexchange.com/questions/32297/resuming-a-partially-completed-encode-with-ffmpeg).
	3. When action doesn't need to do anything, skip instead of executing empty command (media_transcode, media_extract_sub, sub_clone?)
	4. Filter argument to only apply action to filtered files for once
	5. New actions
	...
	8. V2
		1. Setup venv and auto install action-modules when needed
		2. API
		3. Allow adding folders to "monitor and convert"
